  ...
  return (
    <div ref={rootRef} className="fixed inset-x-0 top-4 z-50 px-4 sm:px-6">
      <header className="glass-light mx-auto flex h-14 max-w-6xl items-center justify-between rounded-full pl-5 pr-1.5">
        ...
        <div
          className={`hidden shrink-0 items-center gap-2 lg:flex ${loading ? "pointer-events-none opacity-0" : ""}`}
        >
          {user ? (
            <Link
              href="/dashboard"
          ...
          >
            <div className="grid grid-cols-2 gap-x-8 gap-y-1 xl:grid-cols-3">
              {activeEntry.items.map((item) => (
                ...
                >
                  {item.icon && (
                    <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-brand-accent/10 text-brand-accent">
            ...
            {activeEntry.footerHref && (
              <Link
                ...
                onClick={() => setOpenDesktopMenu(null)}
                className="mt-3 inline-flex items-center gap-1.5 border-t border-ink/10 pt-4 text-[13px] font-medium text-brand-accent-dim"
              >
                      ...
                      <div className="ml-3 flex flex-col gap-1 border-l border-ink/10 pl-3">
                        ...
                        {entry.footerHref && (
                          <Link
                            ...
                            onClick={() => setMenuOpen(false)}
                            className="rounded-md px-3 py-2 text-[13px] font-medium text-brand-accent-dim"
                          >
                            {entry.footerLabel}
                          </Link>
                        )}
