...
/** Renders as the Cal.com booking popup when the CTA is conceptually "book a demo", otherwise a normal Link. */
function CtaLink({
  href,
  className,
  children,
  ...
  if (href === "/book-a-demo") {
    return <CalButton className={className}>{children}</CalButton>;
  }
  return (
      ...
      {steps.map((step, i) => (
        ...
        >
          <div className="glass-light flex w-full flex-col items-center gap-2 rounded-2xl px-5 py-4 text-center sm:w-auto sm:min-w-36">
            {step.icon && (
              <span className="flex h-9 w-9 items-center justify-center rounded-full bg-brand-accent/10 text-brand-accent">
        ...
        <Reveal key={step.index} delay={i * 90}>
          <div className="flex gap-5">
            <div className="flex flex-col items-center">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-accent/10 font-mono text-[13px] font-semibold text-brand-accent-dim">
                {step.index}
              </span>
  ...
  return (
    <Reveal className="mx-auto flex max-w-4xl flex-col items-center gap-3">
      <div className="glass-light flex items-center gap-2 rounded-2xl px-5 py-3">
        {root.icon && <root.icon className="h-4 w-4 text-brand-accent" />}
        <span className="text-[12.5px] font-semibold uppercase tracking-[0.06em] text-ink/75">
          {root.label}
        </span>
      </div>
